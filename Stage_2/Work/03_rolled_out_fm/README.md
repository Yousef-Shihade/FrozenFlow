...
| `rolled_out_fm_models.pt` | Weights for the 3 representative runs per $T$, for step 5 |
...
The brief asks to *compare standard FM training with rolled-out training*. The answer is not
the intuitive one: **removing the train/inference mismatch, on its own, does not produce a
...
**Step 4 — Evaluation.** Assembles the required deliverables from steps 2 and 3's saved
